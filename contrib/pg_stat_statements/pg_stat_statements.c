...
pgss_shmem_startup(void)
{
	...
	/* TODO: Consider using FileGet4 */
	if (FileReadSeq(file, &header, sizeof(uint32), WAIT_EVENT_NONE) != sizeof(uint32) ||
		FileReadSeq(file, &pgver, sizeof(uint32), WAIT_EVENT_NONE) != sizeof(uint32) ||
		FileReadSeq(file, &num, sizeof(int32), WAIT_EVENT_NONE) != sizeof(uint32))
		goto read_error;
	...
	for (i = 0; i < num; i++)
	{
		...
		if (FileReadSeq(file, &temp, sizeof(pgssEntry), WAIT_EVENT_NONE != sizeof(pgssEntry)))
			goto read_error;
		...
		if (FileReadSeq(file, buffer, temp.query_len + 1, WAIT_EVENT_NONE) != temp.query_len + 1)
			goto read_error;
		...
		if (FileWriteSeq(qfile, buffer, temp.query_len + 1, WAIT_EVENT_NONE) != temp.query_len + 1)
			goto write_error;
	...
	if (FileReadSeq(file, &pgss->stats, sizeof(pgssGlobalStats), WAIT_EVENT_NONE) != 1)
		goto read_error;
...
pgss_shmem_shutdown(int code, Datum arg)
{
	...
	if (FileWriteSeq(file, &PGSS_FILE_HEADER, sizeof(uint32), WAIT_EVENT_NONE) != sizeof(uint32))
		goto error;
	if (FileWriteSeq(file, &PGSS_PG_MAJOR_VERSION, sizeof(uint32), WAIT_EVENT_NONE) != sizeof(uint32))
		goto error;
	num_entries = hash_get_num_entries(pgss_hash);
	if (FileWriteSeq(file, &num_entries, sizeof(int32), WAIT_EVENT_NONE) != 1)
		goto error;
	...
	while ((entry = hash_seq_search(&hash_seq)) != NULL)
	{
		...
		if (FileWriteSeq(file, entry, sizeof(pgssEntry), WAIT_EVENT_NONE) != sizeof(pgssEntry) ||
			FileWriteSeq(file, &qstr, len + 1, WAIT_EVENT_NONE) != len + 1)
		{
	...
	if (FileWriteSeq(file, &pgss->stats, sizeof(pgssGlobalStats), WAIT_EVENT_NONE) != sizeof(pgssGlobalStats))
		goto error;
			...
			Size *query_offset, int *gc_count)
{
	...
	*query_offset = off;
	...
	if (FileWrite(fd, query, query_len, off, WAIT_EVENT_NONE) != query_len)
		goto error;
	if (FileWrite(fd, "\0", 1, off + query_len, WAIT_EVENT_NONE) != 1)
		goto error;
...
qtext_load_file(Size *buffer_size)
{
	...
	while (nread < length)
	{
		 ...
		 */
		errno = 0;
		if (FileReadSeq(fd, buf + nread, toread, WAIT_EVENT_NONE) != toread)
		{
			if (errno)
				ereport(LOG,
						(errcode_for_file_access(),
...
gc_qtexts(void)
{
	...
	while ((entry = hash_seq_search(&hash_seq)) != NULL)
	{
		...
		if (FileWriteSeq(qfile, qry, query_len + 1, WAIT_EVENT_NONE) != query_len + 1)
		{
			ereport(LOG,
					(errcode_for_file_access(),
					 errmsg("could not write file \"%s\": %m",
	 ...
	 */
	if (FileTruncate(qfile, extent, WAIT_EVENT_NONE) != 0)
		ereport(LOG,
				(errcode_for_file_access(),
...
entry_reset(Oid userid, Oid dbid, uint64 queryid)
{
	...
	if (FileTruncate(qfile, 0, WAIT_EVENT_NONE) != 0)
		ereport(LOG,
				(errcode_for_file_access(),
				 errmsg("could not truncate file \"%s\": %m",
						PGSS_TEXT_FILE)));
