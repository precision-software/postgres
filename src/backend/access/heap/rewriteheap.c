...
logical_heap_rewrite_flush_mappings(RewriteState state)
{
	...
	while ((src = (RewriteMappingFile *) hash_seq_search(&seq_status)) != NULL)
	{
		 ...
		 */
		written = FWrite(src->vfd, waldata_start, len, src->off,
...
logical_end_heap_rewrite(RewriteState state)
{
	...
	while ((src = (RewriteMappingFile *) hash_seq_search(&seq_status)) != NULL)
	{
		if (!FSync(src->vfd, WAIT_EVENT_LOGICAL_REWRITE_SYNC))
...
heap_xlog_logical_rewrite(XLogReaderState *r)
{
	 ...
	 */
	if (!FTruncate(file, xlrec->offset, WAIT_EVENT_LOGICAL_REWRITE_TRUNCATE))
		ereport(ERROR,
				(errcode_for_file_access(),
	...
	if (FWrite(file, data, len, xlrec->offset, WAIT_EVENT_LOGICAL_REWRITE_MAPPING_WRITE) != len)
		ereport(ERROR,
				(errcode_for_file_access(),
	 ...
	 */
	if (FSync(file, WAIT_EVENT_LOGICAL_REWRITE_MAPPING_SYNC) != 0)
		ereport(data_sync_elevel(ERROR),
				(errcode_for_file_access(),
...
CheckPointLogicalRewriteHeap(void)
{
	...
	while ((mapping_de = ReadDir(mappings_dir, "pg_logical/mappings")) != NULL)
	{
		...
		else
		{
			 ...
			 */
			if (!PathNameFSync(path, WAIT_EVENT_LOGICAL_REWRITE_CHECKPOINT_SYNC))
				ereport(data_sync_elevel(ERROR),
						(errcode_for_file_access(),
							errmsg("could not fsync file \"%s\": %m", path)));
		}
