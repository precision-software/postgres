...
ReorderBufferSerializeTXN(ReorderBuffer *rb, ReorderBufferTXN *txn)
{
	...
	dlist_foreach_modify(change_i, &txn->changes)
	{
			...
			!XLByteInSeg(change->lsn, curOpenSegNo, wal_segment_size))
		{
			 ...
			 */
			file = FOpen(path, PG_ENCRYPT | PG_TRANSIENT | O_CREAT | O_WRONLY | O_APPEND );
			if (file < 0)
				ereport(ERROR,
							...
							File *fileP, XLogSegNo *segno)
{
	...
	while (restored < max_changes_in_memory && *segno <= last_segno)
	{
		...
		if (*fileP == -1)
		{
			...
			*fileP = FOpen(path, PG_ENCRYPT | O_RDONLY | PG_BINARY);
		...
		readBytes = FReadSeq(*fileP, rb->outbuf,
...
typedef struct RewriteMappingF
{
	XLogRecPtr	lsn;
 ...
 */
static void
ApplyLogicalMappingF(HTAB *tuplecid_data, Oid relid, const char *fname)
{
	char		path[MAXPGPATH];
	File		file;
	int			readBytes;
	...
	file = FOpen(path,  PG_ENCRYPT | PG_TRANSIENT | O_RDONLY | PG_BINARY);
	if (file < 0)
		ereport(ERROR,
				(errcode_for_file_access(),
				 errmsg("could not open file \"%s\": %m", path)));
	...
	while (true)
	{
		...
		readBytes = FReadSeq(file, &map, sizeof(LogicalRewriteMappingData), WAIT_EVENT_REORDER_LOGICAL_MAPPING_READ);
...
/*
 * list_sort() comparator for sorting RewriteMappingFs in LSN order.
 */
static int
file_sort_by_lsn(const ListCell *a_p, const ListCell *b_p)
{
	RewriteMappingF *a = (RewriteMappingF *) lfirst(a_p);
	RewriteMappingF *b = (RewriteMappingF *) lfirst(b_p);
...
UpdateLogicalMappings(HTAB *tuplecid_data, Oid relid, Snapshot snapshot)
{
	...
	while ((mapping_de = ReadDir(mapping_dir, "pg_logical/mappings")) != NULL)
	{
		...
		f = palloc(sizeof(RewriteMappingF));
		f->lsn = f_lsn;
	...
	foreach(file, files)
	{
		RewriteMappingF *f = (RewriteMappingF *) lfirst(file);

		elog(DEBUG1, "applying mapping: \"%s\" in %u", f->fname,
			 snapshot->subxip[0]);
		ApplyLogicalMappingF(tuplecid_data, relid, f->fname);
		pfree(f);
	}
}
