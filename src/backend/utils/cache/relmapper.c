...
read_relmap_file(RelMapFile *map, char *dbpath, bool lock_held, int elevel)
{
	...
	snprintf(mapfilename, sizeof(mapfilename), "%s/%s", dbpath,
			 RELMAPPER_FILENAME);
	fd = FOpen(mapfilename, PG_TRANSIENT | PG_ENCRYPT | O_RDONLY | PG_BINARY);
	if (fd < 0)
		ereport(elevel,
	...
	/* Now read the data. */
	r = FReadSeq(fd, map, sizeof(RelMapFile), WAIT_EVENT_RELATION_MAP_READ);
				  ...
				  bool preserve_files, Oid dbid, Oid tsid, const char *dbpath)
{
	 ...
	 */
	fd = FOpen(maptempfilename,
			   PG_TRANSIENT | PG_ENCRYPT | O_WRONLY | O_CREAT | O_TRUNC);
	if (fd < 0)
		ereport(ERROR,
				(errcode_for_file_access(),
				 errmsg("could not open file \"%s\": %m",
	...
	if (FWriteSeq(fd, newmap, sizeof(RelMapFile), WAIT_EVENT_RELATION_MAP_WRITE) != sizeof(RelMapFile))
		ereport(ERROR,
				(errcode_for_file_access(),
