...
static Buffered *bufferedOpen(Buffered *proto, const char *path, int oflags, int perm)
{
	...
	this->bufSize = ROUNDOFF(this->suggestedSize, nextStack(this)->blockSize);
    this->buf = malloc(this->bufSize);

	/* Close our successor if we couldn't allocate memory */
	if (this->buf == NULL)
	{
		checkSystemError(this, -1, "bufferedOpen failed to allocate %zd bytes", this->bufSize);
		return bufferedCleanup(this);
	}
...
static ssize_t bufferedWrite(Buffered *this, const Byte *buf, size_t size, off_t offset)
{
    ...
    if (this->currentSize == 0 && offset == this->currentBlock && size >= this->bufSize)
...
{
	Buffered *this;
	size_t nextSize = thisStack(next)->blockSize;
	size_t blockSize = suggestedSize == 0 ?  nextSize : 1;
	debug("bufferedNew: suggestedSize=%zd  nextSize=%zd  blockSize=%zd\n", suggestedSize, nextSize, blockSize);

    this = malloc(sizeof(Buffered));
    *this = (Buffered)
		{
...
static bool fillBuffer (Buffered *this)
{
	...
	/* Check for holes */
	if (this->sizeConfirmed && this->currentBlock > this->fileSize)
		return setIoStackError(this, -1, "buffereedStack: creating holes (offset=%lld, fileSize=%lld",
                               (long long)this->currentBlock, (long long)this->fileSize);

	/* Read in the current buffer */
	this->currentSize = stackReadAll(nextStack(this), this->buf, this->bufSize, this->currentBlock);
	if (this->currentSize < 0)
		return copyNextError(this, false);

	/* if EOF or partial read, update the known file size */
	this->sizeConfirmed |= (this->currentSize < this->bufSize);
	this->fileSize = MAX(this->fileSize, this->currentBlock + this->currentSize);
