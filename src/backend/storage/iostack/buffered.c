...
static bool bufferedTruncate(Buffered *this, off_t offset);

/**
 * Open a buffered file, reading, writing or both.
 * We always return an I/O stack node, even on failure.
 */
static Buffered *bufferedOpen(Buffered *proto, const char *path, int oflags, mode_t perm)
{
	IoStack *next;
	Buffered *this;
	...
	if (this->buf == NULL)
	{
		stackSetError(this, errno, "bufferedOpen failed to allocate %z bytes", this->bufferSize);
		return bufferedCleanup(this);
	}
...
static ssize_t bufferedWrite(Buffered *this, const Byte *buf, size_t size, off_t offset)
{
    ...
    /* Copy data from caller into our buffer */
 ...
 * TODO: add offset range for writeback.
 */
...
/*
 * Truncate the file at the given offset. TODO: Resize (larger or smaller)
 */
static bool
bufferedTruncate(Buffered *this, off_t offset)
{
	file_debug("bufferedTruncate: offset=%lld file=%zd", offset, this->ioStack.openVal);

	/* Purge our current buffer in case we are altering data below it */
	if (!purgeBuffer(this))
	    return false;
...
{
	file_debug("bufferedSize: size=%lld", this->fileSize);
...
/*
 * Clear out the buffer, saving any existing data.
 */
static bool purgeBuffer(Buffered *this)
{
	bool success = flushBuffer(this);
...
static bool fillBuffer(Buffered *this)
{
	...
	Assert(this->currentBlock % this->bufferSize == 0);
	Assert(this->currentBlock + this->currentSize <= this->fileSize);
	...
	if (this->currentSize < 0)
		return copyNextError(this, false); /* TODO: set current size to zero */
...
static ssize_t copyIn(Buffered *this, const Byte *buf, size_t size, off_t position)
{
	...
	if (this->currentSize == -1) /* TODO: change to Assert and ensure doesn't happen */
		return -1;
...
static ssize_t copyOut(Buffered *this, Byte *buf, size_t size, off_t position)
{
	...
	if (offset > this->currentSize)
	    return stackSetError(this, -1, "Buffered I/O stack hole"); /* TODO: Assert */
