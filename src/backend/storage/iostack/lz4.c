...
off_t getCompressedOffset(Lz4Compress *this, off_t offset)
{
	...
	else if (offset <= this->lastBlock)
	{
		if (!stackReadInt64(this->indexFile, &compressedOffset, this->indexStarts + blockNr * 8))
			compressedOffset = stackSetError(this, -1, "Lz4: offset (%lld) beyond last block (%lld)\n", offset, this->lastBlock);
	}

	/* Otherwise, out of bounds */
	else
		compressedOffset = stackSetError(this, -1, "lz4: Requested offset (%lld), but fileSize is (%lld)\n", offset, this->lastBlock+this->lastSize);

	file_debug("getCompressedOffset: offset=%lld  compressed=%lld\n", offset, compressedOffset);
...
bool fileCopySlice(IoStack *src, off_t srcOffset, size_t size, IoStack *dest, off_t destOffset)
{
	...
	if (total < size && actual == 0)
		stackSetError(src, false, "Unexpected EOF copying compression index\n");
...
{
    file_debug("lz4Open: path=%s  oflags=0x%x\n", path, oflags);
	...
	file_debug("lz4Open: fileSize=%lld  compressedSize=%lld  indexSize=%lld  rawSize=%lld\n", fileSize, compressedSize, indexSize, rawSize);
	...
	file_debug("lz4Open: retval=%d  blockSize=%zd  lastBlock=%lld\n", this->ioStack.openVal, this->blockSize, this->lastBlock);
	return this;
}
...
{
	file_debug("lz4Write: size=%zd offset=%lld lastBlock=%lld\n", size, offset, this->lastBlock);

	/* All writes must be aligned */
	if (offset % this->blockSize != 0)
		return stackSetError(this, -1, "Compression: all writes must be aligned. offset=%lld  alignment=%lld", offset, this->blockSize);

	/* Gaps not allowed */
	if (offset > this->lastBlock + this->blockSize)
		return stackSetError(this, -1, "Compression: holes not allowed.  offset=%lld  fileSize=%lld", offset, this->lastBlock + this->lastSize);

	/* With compression, we can only write (or overwrite) at the end of the file. */
	if (offset + size < this->lastBlock + this->lastSize)
        return stackSetError(this, -1, "Can only write at end of compressed file. offset=%lld  fileSize=%lld", offset, this->lastBlock+this->lastSize);
	...
	if (offset == this->lastBlock + this->lastSize)
	{
		...
		if (!stackWriteInt64(this->indexFile, compressedOffset, indexOffset))
			return stackCheckError(this, -1, "Unable to write to lz4 index file");
...
{
    file_debug("lz4Read: size=%zd  offset=%lld\n", size, offset);
	...
	if (offset % this->blockSize != 0)
		return stackSetError(this, -1, "Compression: all reads must be aligned. offset=%lld  alignment=%lld", offset, this->blockSize);
    ...
    ssize_t compressedActual = stackReadSized(nextStack(this), this->compressedBuf, this->maxCompressed, compressedOffset);
	...
	file_debug("lz4Read(done): actual=%zd lastBlock=%lld  lastSize=%zd\n", actual, this->lastBlock, this->lastSize);
    return actual;
}
...
{
	file_debug("lz4CompressClose: blockSize=%zd\n", this->blockSize);
	...
	if (this->writable)
	{
		...
		file_debug("lz4Close: fileSize=%lld  compressedSize=%lld  indexSize=%lld  rawSize=%lld\n", fileSize, compressedSize, indexSize, rawSize);
...
{
	file_debug("lz4truncate: offset=%lld  lastBlock=%lld  lastSize=%zd", offset, this->lastBlock, this->lastSize);

	/* if readonly, then a permissions error */
	if (!this->writable) {
	    errno = EPERM;
	    return stackCheckError(this, -1, "Truncating a readonly compressed file");
	}
	...
	ssize_t newSize = MIN(offset - this->lastBlock, actual);
...
ssize_t lz4CompressBuffer(Lz4Compress *this, Byte *toBuf, size_t toSize, const Byte *fromBuf, size_t fromSize)
{
	file_debug("lz4CompressBuffer: toSize=%zd fromSize=%zd data=%.*s\n", toSize, fromSize, (int)fromSize, (char*)fromBuf);
...
static Lz4Compress *lz4Cleanup(Lz4Compress *this)
{
	...
	if (index != NULL && index->openVal >= 0)
	{
		...
		if (this->writable && unlink(this->indexPath) < 0)
		    stackCheckError(this, -1, "lz4 Unable to delete index file %s\n", this->indexPath);
	}
...
{
	return stackSetError(this, -1, "Lz4 compression error: code=%d", -retval);
