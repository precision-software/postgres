 ...
 * Purpose
 * --------
 * Provide a uniform set of file access routines layered on
 * top of I/O stacks and virtual file descriptors (VFDs),
 * so encryption and buffering can be injected into the I/O path.
 *
 * Implementation
 * --------------
 * The "injection" is accomplished through layering and renaming.
 * The new code is layered on top of the original Virtual File Descriptors (Vfds),
 * and the new code takes over the names of the original FileRead/FileWrite
 * routines. From the perspective of a caller, nothing has changed, but
 * under the hood the caller invokes the new top layer which eventually
 * invokes the original VFD routines. Encryption, buffering and other
 * transformations take place in "I/O stacks", which fit between the top and
 * bottom layers.  (Note the original VFD routines are renamed by appending
 * "_internal" to their names.)

 * Extensions
 * -----------
 * The top layer creates new routines for doing sequential I/O, making
 * it easier to replace other file access calls with the new ones.
 * These new routines include FileWriteSeq, FileReadSeq, FileSeek
 * and the O_APPEND open flag.
 * Additionally, the new layer defines open flags which
 * can be used to specify encryption, buffering, and eventually
 * compression.
 *
 * Architecture
 * ------------
 * The resulting layers look something like:
 *
 *     __________________________________________________
 *     | New sequential access routines                 |
 *     |   FileReadSeq, FileWriteSeq, O_APPEND          |
 *     --------------------------------------------------
 *                         |
 *     ____________________|_________________________________
 *     | Random Access, same API as virtual file Descriptors|
 *     |      (FileRead, FileWrite ...)                     |
 *     ------------------------------------------------------
 *                         |
 *     ____________________|_____________________________
 *     | I/O Stacks for encryption, buffering, etc      |
 *     --------------------------------------------------
 *                         |
 *     ___________________________________________________
 *     | Virtual File Descriptors, original code renamed |
 *     |   FileRead_internal, FileWrite_internal, ...    |
 *     ---------------------------------------------------
 *
 * Error Handling
 * --------------
 *  If an I/O stack error occurs, say for encryption,
 *  the existing calling code probably won't know what to do with it.
 *  We throw an error exception and let the caller deal with it.
 *  Normal file errors are returned as -1 with errno for compatibility,
 *  The caller can also call FileErrorCode(vfd)
 *  to get the error code, and FileErrorCode(-1) contains info about
 *  the last open call which failed to open a vfd.
 */
...
void fileSetError(File file, int errnum, const char *fmt, ...) pg_attribute_printf(3,4);
...
{
    int retval = -1;
    IoStack saveError = {0};  /* Place to save previous error info if any */
    IoStack *ioStack;
	...
	ioStack = (file < 0) ? NULL : getStack(file);  /* Don't throw exception on -1 */
	if (ioStack == NULL)
	...
	/* No matter what, release the residual I/O stack element */
	if (ioStack != NULL)
	{
		free(ioStack);
...
ssize_t FileRead(File file, void *buffer, size_t amount, off_t offset, uint32 wait_event_info)
{
	...
	/* Get the I/O stack associated with this file */
	stack = getStack(file);

	/* Read the data from the stack as requested */
	pgstat_report_wait_start(wait_event_info);
	actual = stackReadAll(stack, buffer, amount, offset);
...
int	FileTruncate(File file, off_t offset, uint32 wait_event_info)
{
	...
	pgstat_report_wait_start(wait_event_info);
	retval = stackTruncate(getStack(file), offset);
	pgstat_report_wait_end();
	throwIoStackError(file);
	return (int)retval;
}
...
{
	char c;
	int ret = FileReadSeq(file, &c, 1, 0);
	if (ret <= 0)
		ret = EOF;
	return ret;
}
...
static IoStack *selectIoStack(const char *path, int oflags, int mode)
{
	...
	switch (oflags & PG_STACK_MASK)
	{
		...
		default: ereport(FATAL,
                         errcode(ERRCODE_INTERNAL_ERROR),
                         errmsg("Unrecognized I/O Stack oflag 0x%x", (oflags & PG_STACK_MASK)));
	}
}


static uint64 getTempSequenceNr()
{
	return GetFakeLSNForUnloggedRel();
}

static uint64 getPermSequenceNr()
{
	return GetFakeLSNForUnloggedRel();
}
...
void ioStackSetup(void)
{
	...
	ioStackRaw = vfdStackNew(1);
	ioStackEncrypt =     bufferedNew(1, aeadNew("AES-256-GCM", 16 * 1024, tempKey, tempKeyLen, getTempSequenceNr, vfdStackNew(BLCKSZ)));
	ioStackEncryptPerm = bufferedNew(1, aeadNew("AES-256-GCM", 16 * 1024, permKey, permKeyLen, getPermSequenceNr, vfdStackNew(BLCKSZ)));
	ioStackPlain = bufferedNew(1, vfdStackNew(8*1024));
	ioStackCompress = bufferedNew(1, lz4CompressNew(64 * 1024, vfdStackNew(1), vfdStackNew(1)));
	ioStackCompressEncrypt = bufferedNew(1,
										 lz4CompressNew(16*1024,
														bufferedNew(1,
																	aeadNew("AES-256-GCM", 16 * 1024, tempKey, tempKeyLen, getTempSequenceNr,
																			vfdStackNew(1))),
														bufferedNew(1,
																	aeadNew("AES-256-GCM", 16 * 1024, tempKey, tempKeyLen, getTempSequenceNr,
																			vfdStackNew(1)))));
