/*
 * Routines for accessing Temp and other non-relational files.
 */
#include "postgres.h"
 ...
 * TODO: Invoke from FileClose so close() infrastructure works.
 */
 ...
 */
bool FSync(File file, off_t offset, size_t amount, uint32 wait)
{
	bool success;
	...
	success = stackSync(getStack(file), wait);
...
bool	FResize(File file, off_t offset, uint32 wait)
{
	...
	if (badFile(file))
		return -1;
	success = (int)stackResize(getStack(file), offset, wait);
